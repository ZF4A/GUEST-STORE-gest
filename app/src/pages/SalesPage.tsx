...
export default function SalesPage() {
  ...
  const { data, isLoading } = trpc.sales.list.useQuery({ page, limit: 20 });
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-b border-white/[0.04]">
                  <td colSpan={4} className="px-5 py-4"><div className="h-4 bg-white/[0.04] rounded w-3/4 animate-pulse" /></td>
                </tr>
              ))
            ) : data?.items.length === 0 ? (
              <tr>
                <td colSpan={4} className="text-center py-16">
                  <div className="flex flex-col items-center text-white/30">
                    <Receipt className="w-12 h-12 mb-3" strokeWidth={1} />
                    <p>{t("No sales recorded")}</p>
                  </div>
                </td>
              </tr>
            ) : (
                ...
                <tr key={sale.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-white">{sale.employeeName}</span>
                      <span className="text-[10px] text-white/30 font-mono-id bg-white/[0.06] px-1.5 py-0.5 rounded">{sale.employeeCode}</span>
                    </div>
                  </td>
